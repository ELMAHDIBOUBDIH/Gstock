...
<table class="table table-striped table-bordered" style="min-width:73%;width:auto;    margin-left: 13%;">
    ...
    <tr style="background-color:rgba(2, 142, 0, 0.41);">
        <form method="POST" action="{{route('dashboard::Invoice.update',$Invoice->id)}}" >
        {{csrf_field() }}
            
            <td>
                <div class="input-group  col-md-8">
                    <input type="text" class="form-control" name="idfacture"   value="{{$Invoice->reference_cmd}}" disabled>
            </div>
        </td>
        <td>
        ...
        <td>
        <a href="{{route('dashboard::Invoice.index')}}" title=""><button type="button" class="btn btn-danger" id="close" data-dismiss="modal">annuler</button></a></td>
    </tr>
</form>
</tbody>
</table>
